...
Before `REPORTING`, the runner also applies a report-quality gate over the persisted ledger. The
gate checks reportable supported claim count, support-evidence count, source-document count,
coverage sufficiency, source diversity, and unresolved required slots. If those thresholds fail
while search/fetch/round budgets remain, the failure becomes a bounded `report_quality_gate`
supplemental search round rather than a thin final report. Runtime claim-drafting and verification
batch limits are aligned with `RESEARCH_CLAIM_LIMIT`, so normal research runs no longer silently
cap the evidence funnel at roughly a dozen claims before reporting.
