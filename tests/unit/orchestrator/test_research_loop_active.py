...
def test_strategist_continue_search_suppressed_when_only_optional_weak_slots_remain(
    mock_session: MagicMock, mock_task: MagicMock
) -> None:
    mock_task_repo = MagicMock()
    mock_task_repo.get.return_value = mock_task
    mock_task.revision_no = 1

    mock_strategist = MagicMock(spec=LLMResearchStrategistService)
    mock_strategist.enabled = True
    planned = [{"query_text": "privacy follow-up", "metadata": {}}]
    ...
    mock_result.to_payload.return_value = {
        "status": "used",
        "decision": "continue_search",
        "planned_queries": planned,
    }
    mock_strategist.decide.return_value = mock_result
    ...
    runner._current_coverage_evaluation = MagicMock(  # type: ignore
        return_value={
    ...
    runner._current_report_quality_gate = MagicMock(return_value={"triggered": False})  # type: ignore
    runner._existing_search_query_texts = MagicMock(return_value=set())  # type: ignore
    runner._run_research_more_round = MagicMock(return_value={"status": "ok"})  # type: ignore

    runner.run(mock_task.id)

    assert runner._run_research_more_round.called is False
    gap_calls = [
        c
        for c in runner.event_repository.record.call_args_list
        if c.kwargs.get("event_type", "").endswith("gap_analysis")
    ]
    assert gap_calls, "expected a gap_analysis event"
    last_gap = gap_calls[-1].kwargs["payload_json"]["result"]
    assert last_gap.get("loop_stop_reason") == COVERAGE_SUFFICIENT_OPTIONAL_WEAK_ONLY_STOP
